...
| ------------ | ------------------------------- | -------------------------------- |
| Bot + Caddy  | VM Oracle `E2.1.Micro`, `/opt/cobot` | `ssh cobot`, Docker Compose |
| Painel       | Vercel                          | painel da Vercel / `git push`    |
| Postgres     | Supabase                        | painel do Supabase               |

O apelido `cobot` no `~/.ssh/config` é o que os comandos abaixo assumem.
...
```bash
ssh cobot 'cd /opt/cobot && docker compose logs -f --tail 200 bot'
ssh cobot 'cd /opt/cobot && docker compose logs --since 1h bot | grep '"'"'"level":50'"'"''  # só erros
```

**Caddy (acessos e TLS).** É aqui que aparece um 401 e é daqui que o fail2ban lê:

```bash
ssh cobot 'cd /opt/cobot && docker compose logs --tail 200 caddy'
```

**Painel (Vercel).** Painel do projeto → *Logs*, ou `vercel logs <deployment>`.
Um erro de server action aparece com o stack; um `redirect` para `/denied` não
é erro.

**Backup.** `ssh cobot 'cd /opt/cobot && docker compose logs backup'`.
...
ssh cobot 'cd /opt/cobot && docker compose restart bot'

# Tudo.
ssh cobot 'cd /opt/cobot && docker compose down && docker compose up -d'

# Voltar para uma imagem anterior (rollback) — a tag sai do GHCR.
ssh cobot 'cd /opt/cobot && TAG=sha-1a2b3c4 docker compose up -d bot'
```

O painel na Vercel não se "reinicia": use *Redeploy* no deployment que
funcionava.
...
```bash
# 1. Ver o que existe.
ssh cobot 'cd /opt/cobot && docker compose exec backup ls -lah /backups'

# 2. Trazer o dump escolhido para a máquina local.
ssh cobot 'cd /opt/cobot && docker compose exec -T backup cat /backups/daily-20260907-060000.sql.gz' > dump.sql.gz

# 3. Restaurar num banco DESCARTÁVEL primeiro. Sempre.
docker compose -f infra/docker-compose.dev.yml up -d postgres
docker compose -f infra/docker-compose.dev.yml exec postgres createdb -U cobot cobot_restore
infra/scripts/restore.sh dump.sql.gz postgres://cobot:cobot@localhost:5432/cobot_restore

# 4. Conferir que o schema está na última versão.
DATABASE_URL=postgres://cobot:cobot@localhost:5432/cobot_restore pnpm db:migrate
```
...
1. **VM.** `ssh cobot`, edite `/opt/cobot/.env` (`INTERNAL_API_TOKEN=$NOVO`) e
   `cd /opt/cobot && docker compose up -d bot`. O bot reinicia em segundos; o
   painel fica com 401 nesse intervalo e volta sozinho.
2. **Vercel.** Projeto → *Settings* → *Environment Variables* → editar
   `INTERNAL_API_TOKEN` → *Redeploy* do último deployment de produção.
3. **GitHub.** *Settings* → *Secrets and variables* → *Actions* → atualizar
...
1. Developer Portal → *Bot* → *Reset Token*. A partir daí o token antigo morre:
   o bot cai. Isto **tem** downtime.
2. `ssh cobot`, edite `DISCORD_TOKEN` em `/opt/cobot/.env`.
3. `cd /opt/cobot && docker compose up -d bot`.
...
*OAuth2*, e atualizado nas variáveis da Vercel.
...
3. Código `4014` = intent privilegiada desligada → Developer Portal → *Bot* →
   ligue *Server Members* e *Message Content* (PRD §7.3).
...
1. Painel do Supabase → *Restore project*. Leva alguns minutos.
...
```bash
ssh cobot 'sudo fail2ban-client status cobot-api'      # IPs banidos
ssh cobot 'sudo fail2ban-client set cobot-api unbanip 1.2.3.4'  # desbanir
```
...
- [ ] **Espaço em disco:** `ssh cobot 'df -h /'` — abaixo de 80%.
- [ ] **Memória:** `ssh cobot 'cd /opt/cobot && docker stats --no-stream'` —
...
- [ ] **Cota do Supabase:** painel → *Settings* → *Usage*. O free tier são
      500 MB; acima de 400 MB, revise as retenções (PRD §8).
- [ ] **Uso da Vercel:** painel → *Usage*. Um servidor só fica muito abaixo do
      limite; um salto quer dizer que alguém achou o painel.
- [ ] **fail2ban:** `sudo fail2ban-client status cobot-api` — banimentos de
...
ssh cobot 'cd /opt/cobot && docker compose ps && docker stats --no-stream'
```
