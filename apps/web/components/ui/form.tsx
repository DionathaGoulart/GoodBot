...
function useFormField() {
  ...
  if (!fieldContext || !itemContext) {
    throw new Error('useFormField precisa estar dentro de <FormField>/<FormItem>.');
  }
...
/** §6.4 — a ajuda vem antes do campo, logo abaixo do label. */
function FormDescription({ className, ...props }: React.ComponentProps<'p'>) {
  ...
  return (
    <p
      ...
      className={cn('text-xs opacity-60', className)}
      {...props}
    />
  );
}

/** §6.4 — erro é micro-texto `error` prefixado por `!`. */
