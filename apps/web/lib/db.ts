...
const globalForDb = globalThis as unknown as { cobotDb?: ReturnType<typeof createDb> };

function connection() {
  globalForDb.cobotDb ??= createDb(env().DATABASE_URL, { max: 1, idleTimeout: 10 });
  return globalForDb.cobotDb;
}
