...
export const guildCreate = defineEvent(
  ...
  // Este é o evento que **cria** a linha `pending`: gateá-lo pelo registro
  // seria exigir que a guild já estivesse no registro para entrar nele.
  { always: true },
);
...
export const guildDelete = defineEvent(
  ...
  // A saída de uma guild não atendida (bloqueada, demo vencida) também precisa
  // ser registrada — é justamente ela que sai.
  { always: true },
);
