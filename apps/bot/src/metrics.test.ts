...
describe('Counter', () => {
  it('soma por conjunto de labels e expõe cada série', () => {
    const counter = new Counter('cobot_x_total', 'ajuda');
    counter.inc({ tipo: 'a' });
    counter.inc({ tipo: 'a' });
    ...
    expect(text).toContain('cobot_x_total{tipo="a"} 2');
    expect(text).toContain('cobot_x_total{tipo="b"} 3');
  });

  it('sem série nenhuma ainda expõe o zero', () => {
    expect(new Counter('cobot_y_total', 'ajuda').render()).toContain('cobot_y_total 0');
  });

  it('escapa aspas no valor da label', () => {
    const counter = new Counter('cobot_z_total', 'ajuda');
...
describe('Summary', () => {
  it('calcula p50 e p95 das amostras', () => {
    const summary = new Summary('cobot_lat_ms', 'ajuda');
    ...
    expect(text).toContain('cobot_lat_ms_count 100');
    expect(text).toContain('quantile="0.5"');
  });

  it('sem amostra não inventa percentil', () => {
    expect(new Summary('cobot_vazio', 'ajuda').quantile(0.5)).toBeNull();
  });
});
...
describe('Gauge', () => {
  it('lê o valor na hora da renderização', () => {
    let value = 1;
    const gauge = new Gauge('cobot_fila', 'ajuda');
    gauge.register(() => value, { fila: 'log' });

    expect(gauge.render().join('\n')).toContain('cobot_fila{fila="log"} 1');
    value = 7;
    expect(gauge.render().join('\n')).toContain('cobot_fila{fila="log"} 7');
  });
  ...
  it('um leitor que explode não derruba o scrape', () => {
    ...
    expect(text).toContain('cobot_fila{fila="boa"} 2');
...
describe('renderMetrics', () => {
  it('sai no formato de exposição, com HELP e TYPE e quebra final', () => {
    ...
    expect(text).toMatch(/^# HELP cobot_events_total /);
    expect(text).toContain('# TYPE cobot_api_duration_ms summary');
...
describe('teto de séries', () => {
  it('descarta séries novas depois do teto, sem perder as que já existem', () => {
    const counter = new Counter('cobot_muitos_total', 'ajuda');
    for (let i = 0; i < 300; i += 1) counter.inc({ rota: `/r${String(i)}` });
    counter.inc({ rota: '/r0' });

    // 256 séries no máximo, e a primeira continua contando.
    expect(counter.render().filter((line) => line.startsWith('cobot_muitos_total{'))).toHaveLength(
      256,
    );
    expect(counter.render().join('\n')).toContain('cobot_muitos_total{rota="/r0"} 2');
  });

  it('o mesmo vale para o resumo de latência', () => {
    const summary = new Summary('cobot_muitos_ms', 'ajuda');
